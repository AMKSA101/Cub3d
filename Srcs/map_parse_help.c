...
/*   Updated: 2025/04/12 09:36:55 by abamksa          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int is_valid_zero(char **map, size_t i, size_t j, t_scene *scene)
{
	...
	if (j == 0 || j == map_width - 1)// Boundary checks for current line
		return 0;
	if (i > 0) // Check above line
	{
		prev_line_width = ft_strlen(map[i - 1]);
		if (j >= prev_line_width || !is_wall_or_player(map[i - 1][j])) return 0;
	}
	if (i + 1 < scene->map_height) // Check below line
	{
		next_line_width = ft_strlen(map[i + 1]);
		if (j >= next_line_width || !is_wall_or_player(map[i + 1][j])) return 0;
	}
	if (!is_wall_or_player(map[i][j - 1]) || !is_wall_or_player(map[i][j + 1])) // Check adjacent tiles in the current line
		return 0;
	return 1; // Valid '0'
}
